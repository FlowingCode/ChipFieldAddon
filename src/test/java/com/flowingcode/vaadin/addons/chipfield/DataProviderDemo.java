...
public class DataProviderDemo extends VerticalLayout {
  ...
  public DataProviderDemo() {
    ...
    ChipField<Planet> chf =
        new ChipField<>(
            "Select some planets (Mercury, Venus, Earth, etc.)", planet -> planet.getName());
    chf.setWidthFull();
    ...
    chf.addChipCreatedListener(
        ev ->
            Notification.show(
                "Chip: " + ev.getItem() + " Created by client: " + ev.isFromClient() + "!",
                5000,
                Position.BOTTOM_START));
    chf.addChipRemovedListener(
        ev ->
            Notification.show(
                "Chip: " + ev.getItem() + " Removed by client: " + ev.isFromClient() + "!",
                5000,
                Position.BOTTOM_START));
